...
def main() -> None:
    ...
    tabs = st.tabs(["Overview", "Performance", "Holdings", "Transactions & Income",
                    "Tax (NL Box 3)", "Data"])
    ...
    with tabs[0]:
        ...
        c3.metric("Total P/L", f"{k.get('total_pnl', 0):,.0f} {base}",
        ...
        fig.add_trace(go.Scatter(x=df["date"], y=df["net_invested"],
                                 name="Net invested", line=dict(dash="dash")))
        ...
        cgfig = go.Figure()
        cgfig.add_trace(go.Scatter(x=cg["date"], y=cg["contributions"], name="Contributions",
                                   stackgroup="one", line=dict(width=0.5)))
        cgfig.add_trace(go.Scatter(x=cg["date"], y=cg["market_growth"], name="Market growth",
                                   stackgroup="one", line=dict(width=0.5)))
        cgfig.update_layout(height=320, hovermode="x unified")
        st.plotly_chart(cgfig, use_container_width=True)
        st.caption("How much of your total value is money you added vs. market gains. "
    ...
    with tabs[1]:
        ...
        if curves.empty:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=curves["date"], y=curves["twr_pct"],
                                     name="TWR (Time-Weighted Return)"))
            fig.add_trace(go.Scatter(x=curves["date"], y=curves["pl_vs_invested_pct"],
                                     name="P/L vs invested", line=dict(dash="dash")))
            # Benchmark overlay(s)
            bench = data["benchmark_curves"]
            for name, grp in bench.groupby("benchmark"):
                fig.add_trace(go.Scatter(x=grp["date"], y=grp["return_pct"],
                                         name=f"Benchmark: {name}",
                                         line=dict(dash="dot", color="gray")))
            fig.update_layout(title="Return over time (%) — vs benchmark",
                              hovermode="x unified", height=440, yaxis_ticksuffix="%")
            st.plotly_chart(fig, use_container_width=True)
            if not bench.empty:
                st.caption("Benchmark = a buy-and-hold index for comparison (set in "
            ...
            if "pl_vs_invested" in curves:
                plfig = go.Figure()
                plfig.add_trace(go.Scatter(
                    x=curves["date"], y=curves["pl_vs_invested"],
                    name="P/L vs invested", fill="tozeroy",
                    line=dict(color="#2e9e5b")))
                plfig.update_layout(
                ...
                st.caption("Absolute profit/loss in cash terms: how many "
        ...
        if not dd.empty:
            ...
            ddfig = px.area(dd, x="date", y="drawdown_pct", height=280,
                            labels={"drawdown_pct": "Drawdown %"})
            ...
            st.caption("Decline from the portfolio's running peak (deposit-proof, from the "
        ...
        if risk:
            ...
            r1.metric("Volatility (ann.)", f"{risk['volatility_pct']:.1f}%",
            ...
            r2.metric("Return (ann.)", f"{risk['ann_return_pct']:.1f}%",
            ...
            r4.metric("Max drawdown", f"{risk['max_drawdown_pct']:.1f}%",
            ...
            st.caption(f"Annualised from ~{risk['days']} days of daily TWR returns — "
        ...
        st.caption("How similarly your holdings move (effective overlap). True constituent-level "
        ...
        if corr.empty or corr.shape[0] < 2:
            ...
            cfig = px.imshow(cshort, text_auto=".2f", color_continuous_scale="RdBu_r",
    ...
    with tabs[3]:
        ...
        if tx.empty:
            ...
            if query:
                mask = (tx["name"].fillna("").str.lower().str.contains(query)
                        | tx["symbol"].fillna("").str.lower().str.contains(query))
                tx = tx[mask]
        ...
        if dy.empty:
            ...
            st.dataframe(
                dy, use_container_width=True, hide_index=True,
                column_config={
            ...
            st.caption("Trailing = dividends received so far ÷ current value. Not annualised "
...
def _holdings_detail(data, base):
    ...
    if ph.empty:
        ...
        st.plotly_chart(
            px.line(ph, x="date", y="value", color="name", height=400,
    ...
    if prh.empty:
        ...
        fig_rh = px.line(prh, x="date", y="return_pct", color="name", height=400,
    ...
    if perf.empty:
        ...
        fig_perf = px.bar(
            perf, x="return_pct", y="name", orientation="h", height=360,
            color="color", color_discrete_map={"gain": "#2e9e5b", "loss": "#d6455d"},
            labels={"return_pct": "Return %", "name": ""}, text_auto=".1f",
        )
    ...
    with a1:
        ...
        st.plotly_chart(px.pie(by_cat, names="category", values="value",
                               title="Core vs satellite", hole=0.4), use_container_width=True)
    with a2:
        by_region = cls.groupby("region", as_index=False)["value"].sum()
        st.plotly_chart(px.pie(by_region, names="region", values="value",
                               title="By region", hole=0.4), use_container_width=True)
    with a3:
        st.plotly_chart(px.pie(cls, names="theme", values="value",
                               title="By theme", hole=0.4), use_container_width=True)
    st.caption("Classification from holdings_meta.yml — edit that file to reclassify. "
...
def _data_tab(data, base):
    ...
    if unresolved:
        st.warning(f"{len(unresolved)} product(s) without a resolved price ticker — "
                   "add them to tickers.yml:")
...
def _download(col, label, df, filename):
    if df is None or df.empty:
        ...
        return
    col.download_button(label, df.to_csv(index=False).encode("utf-8"),
...
def _income_section(df, base, cumulative: bool):
    ...
    st.dataframe(
        df.sort_values("month"), use_container_width=True, hide_index=True,
    ...
    bar = px.bar(df, x="month", y="amount", height=300,
                 color="currency" if multi_ccy else None)
    ...
    if cumulative:
        ...
        st.plotly_chart(
            px.line(cum, x="month", y="cumulative", markers=True, height=260,
                    labels={"cumulative": f"Cumulative ({base})"}),
            use_container_width=True,
        )
...
def _tax_tab(data, base):
    ...
    if cc.empty:
        ...
        st.caption(f"Official reports fetched: **{fetched}**. App figures vs DEGIRO's own "
                   "account statement & position report.")
        st.dataframe(
            cc, use_container_width=True, hide_index=True,
            column_config={
        ...
        if not cch.empty:
            ...
            st.dataframe(
                cch, use_container_width=True, hide_index=True,
                column_config={
                    "app": st.column_config.NumberColumn(f"App ({base})", format="%.2f"),
                    "official": st.column_config.NumberColumn(f"Official ({base})", format="%.2f"),
                    "delta": st.column_config.NumberColumn(f"Δ ({base})", format="%.2f"),
        ...
        d1, d2 = st.columns(2)
        if acct_path.exists():
            d1.download_button("⬇ Account statement (CSV)", acct_path.read_bytes(),
                               file_name="degiro_account_report.csv", mime="text/csv")
        if pos_path.exists():
            d2.download_button("⬇ Position report (CSV)", pos_path.read_bytes(),
                               file_name="degiro_position_report.csv", mime="text/csv")
    ...
    if ref.empty:
        ...
        st.dataframe(
            ref, use_container_width=True, hide_index=True,
            column_config={"value": st.column_config.NumberColumn(
                f"Value 1-Jan ({base})", format="%.2f")},
        )

    # 2. Rough Box 3 estimate
    st.subheader("Rough Box 3 estimate")
    st.caption("Parameters change yearly — defaults shown for reference, edit as needed. "
    ...
    value = c1.number_input(f"Asset value on 1 Jan ({base})", min_value=0.0,
                            value=round(latest_value, 2), step=1000.0,
    ...
    allowance = c5.number_input("Tax-free allowance (€)", min_value=0.0,
                                value=51396.0, step=100.0)
    if partners:
        allowance *= 2
    ...
    if div.empty:
        ...
        st.caption("No Dutch dividend withholding (*dividendbelasting*) is recorded in your "
    ...
    if rg.empty:
        ...
        st.caption("FIFO (First-In, First-Out) matched. Not taxed for NL private investors "
