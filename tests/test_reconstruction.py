...
def _seed_basic(buy_day: str):
    ...
    with store.connection() as conn:
        store.save_products(conn, {1: {"isin": "TEST", "symbol": "TESTX",
                                       "name": "Test", "currency": "EUR"}})
        store.save_transactions(conn, [{
            "id": 1, "date": buy_day, "product_id": 1, "buysell": "B",
            "quantity": 10.0, "price": 100.0, "total": -1000.0,
            "total_in_base_currency": -1000.0,
            "total_plus_all_fees_in_base_currency": -1001.0,
            "fee_in_base_currency": -1.0,
        }])
        store.save_cash_movements(conn, [
            {"id": 99, "date": buy_day, "value_date": buy_day, "product_id": None,
             "type": "CASH_TRANSACTION", "description": "flatex Deposit",
             "currency": "EUR", "change": 1000.0, "balance": {}},
            # The purchase debits cash (real data records this as a TRANSACTION row):
            {"id": 101, "date": buy_day, "value_date": buy_day, "product_id": 1,
             "type": "TRANSACTION", "description": "Compra 10 Test",
             "currency": "EUR", "change": -1000.0, "balance": {}},
            # Internal sweep must NOT count toward cash:
            {"id": 100, "date": buy_day, "value_date": buy_day, "product_id": None,
             "type": "FLATEX_CASH_SWEEP", "description": "Degiro Cash Sweep Transfer",
             "currency": "EUR", "change": 950.0, "balance": {}},
        ])
...
def test_snapshot_overrides_reconstruction(tmp_db):
    ...
    with store.connection() as conn:
        store.save_value_snapshot(conn, {"date": snap_day, "holdings_value": 12345.0,
                                         "cash": 5.0, "total_value": 12350.0,
                                         "net_invested": 1000.0})
...
def test_fifo_realized_gains(tmp_db):
    with store.connection() as conn:
        ...
        store.save_transactions(conn, [
            {"id": 1, "date": "2026-01-01", "product_id": 1, "buysell": "B", "quantity": 10.0,
             "price": 100.0, "total": -1000.0, "total_in_base_currency": -1000.0,
             "total_plus_all_fees_in_base_currency": -1000.0, "fee_in_base_currency": 0.0},
            {"id": 2, "date": "2026-02-01", "product_id": 1, "buysell": "S", "quantity": -4.0,
             "price": 150.0, "total": 600.0, "total_in_base_currency": 600.0,
             "total_plus_all_fees_in_base_currency": 600.0, "fee_in_base_currency": 0.0},
        ])
